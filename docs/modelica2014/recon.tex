...
The \code{"tabs"} key is associated with the list of tables present in
...
associated with a value which is, in turn, a \msgpack map.  The keys
in this map represent the names of objects present in the \wall file
and the values represent the metadata associated with those objects.
{\b Note}, the metadata values here are not optional (like it is with
variables in tables).  If there is no metadata associated with a given
object, an empty map must be included as a value.

\subsubsection{Data Entries}

After the header, the remainder of the file are ``data entry''
values.  Each entry starts with a 4 byte network order integer
representing the length of the entry followed by that number of bytes
stored as a serialized \msgpack map.

There are two types of entries.  The first type is a ``row entry''
which details a new row for a specified table.  The format of a row
entry is as follows:
...
The complete value of a given object
is determined by processing all field entries associated with that
object in the order they appear in the \wall file.  One consequence of
this is that it is therefore possible to have the values of individual
fields change while writing the file.  The true value is simply the
once associated with the final entry.
